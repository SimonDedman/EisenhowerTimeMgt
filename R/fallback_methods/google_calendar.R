...
get_calendar_events <- function(calendar_id = "primary",
  ...
  events_df <- data.frame(
    id = ifelse(is.null(events_data$items$id), NA, events_data$items$id),
    summary = ifelse(is.null(events_data$items$summary), NA, events_data$items$summary),
    description = ifelse(is.null(events_data$items$description), NA, events_data$items$description),
    start_time = ifelse(is.null(events_data$items$start$dateTime), 
                       ifelse(is.null(events_data$items$start$date), NA, events_data$items$start$date),
                       events_data$items$start$dateTime),
    end_time = ifelse(is.null(events_data$items$end$dateTime),
                     ifelse(is.null(events_data$items$end$date), NA, events_data$items$end$date),
                     events_data$items$end$dateTime),
