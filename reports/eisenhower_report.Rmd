...
```{r work-by-list, results='asis'}
if (nrow(work_task_data) > 0 && "list_name" %in% colnames(work_task_data)) {
  
  # Get unique lists in work data
  work_lists <- work_task_data %>%
    filter(!is.na(list_name)) %>%
    group_by(list_name) %>%
    summarise(count = n(), .groups = "drop") %>%
    arrange(desc(count))
  
  if (nrow(work_lists) > 0) {
    for (i in 1:nrow(work_lists)) {
      list_name <- work_lists$list_name[i]
      list_data <- work_task_data %>% filter(list_name == !!list_name)
    ...
    cat("No list information available for work tasks.\n")
  }
} else {
  cat("No work list data available.\n")
}
```
